...
def choose_hotel_h(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, HighLowFunc.country_h, message.chat.id)


@bot.message_handler(state=[HighLowFunc.country_h])
def get_country_h(message: Message) -> None:
    ...
    if message.text.isalpha():
        bot.send_message(message.from_user.id, 'Спасибо записал. Теперь введи город,'
                                               ', и я выведу самый дорогой отель в нем')
        bot.set_state(message.from_user.id, HighLowFunc.city_h, message.chat.id)
...
@bot.message_handler(state=[HighLowFunc.city_h])
...
def get_hotels_list_h(message: Message) -> None:
    ...
    :get_hotels_list: функция, на вход подается сообщение
                  ...
                  При неудачной попытке запроса предлагается начать снова.

                  В случае успеха формируется список с найденными отелями
    ...
    try:
        ...
        info = deteil_info(hotels_list[-1][0])
        hotel = hotels_list[-1]
